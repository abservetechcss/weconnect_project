...
function CreateFunnelHeaderComponent(props) {
  const [startDate, endDate] = props.createFunnel
    ? props.createFunnelDate
    : props.filterDate;
  const [state, setState] = React.useState({
    right: false,
  });
  const onSearchText = () => {
    let data = props.question_lists;
    ...
    if (
      ...
      let searchResult =
        ...
        data.length > 0 &&
        data.filter((x) => {
          return x.question.toLowerCase().includes(searchText);
        });
      props._this.setState({
        loading: false,
        question_lists: searchResult,
      });
    } else {
      props._this.setState({
  ...
  return (
    <Fragment>
      <div className="submain_header lead-sub-header-block insight-sub-header">
        <div className="search_date_block created_funnel_block">
          <div className="search-block funnel_inner_header">
            {/* <Autocomplete
              disablePortal
              id="combo-box-demo"
              name="selectBotId"
                    ...
                    >
                      <div className="edit_question_drawer">
                        <div className="edit_question_header">
                          <p className="created_heading">Created Funnel</p>
                          <p
                            role="button"
                            onClick={() => {
                              setState({ right: false });
                        ...
                        <div className="funnel_main">
                          <FormGroup>
                            {props.question_lists &&
                            props.question_lists.length > 0 ? (
                              props.question_lists.map((prop, index) => {
                                return (
                                  <FormControlLabel
                                    control={
                                      <Checkbox
...
export default CreateFunnelHeaderComponent;
