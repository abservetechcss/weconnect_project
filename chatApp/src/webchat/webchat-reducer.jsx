...
export function webchatReducer(state, action) {
  ...
  switch (action.type) {
    ...
    case UPDATE_THEME:
      if (
        Array.isArray(action.payload.messagesComponents) &&
        action.payload.messagesComponents.length === 0
      ) {
        delete action.payload.messagesComponents;
      }
      return {
        ...state,
        ...action.payload,
    ...
    default:
      const messageReducer = messagesReducer(state, action);
