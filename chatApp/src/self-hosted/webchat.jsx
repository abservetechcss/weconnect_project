...
import { Text, Image, Audio, Document, Video } from '../actions/components';
...
import { EmojiPicker, OpenedEmojiPicker } from '../webchat/components/emoji-picker'
import {
...
export const Webchat = forwardRef((props, ref) => {
  ...
  const messageComponentFromInput = input => {
    ...
    if (isText(input)) {
      ...
      messageComponent = (
        <Text html={input.html} id={input.id} payload={input.payload} from={sender}>
          {input.data}
        </Text>
      )
  ...
  */
  const setHandoff = (handoff) => {
    if (handoff)
      updateSession({ ...webchatState.session, _botonic_action: `create_case:""` });
    else 
      updateSession({ ...webchatState.session, _botonic_action: "" });
  };
  ...
  useImperativeHandle(ref, () => ({
    addBotResponse: async ({ response, session, lastRoutePath }) => {
      ...
      if (Array.isArray(response)) response.map(r => {
  ...
  const sendTextAreaText = async () => {
    ...
    if (props.onValidateInput)
      result = await props.onValidateInput({ input: textArea.current.value, elem: textArea, webchatState: webchatState,
        lastRoutePath: webchatState.lastRoutePath, });
    if (result) {
  ...
  const webchatMessageList = () => (
    <WebchatMessageList style={{ flex: 1, display: webchatState.isCoverComponentOpen?'none':'' }}>
      <MessageContainer> 
  ...
  const webchatReplies = () => <WebchatReplies style={ {width: "100%"}} replies={webchatState.replies} />
